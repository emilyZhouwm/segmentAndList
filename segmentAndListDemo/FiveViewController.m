...
- (void)openList:(NSInteger)segmentIndex
{
    ...
    if (!listView) {
        ...
        __weak typeof(self) weakSelf = self;
        WMDropDown2View *listView = [[WMDropDown2View alloc] initWithFrame:rect
                                                                    titles:lists
                                                              defaultIndex:[_totalIndex[segmentIndex] integerValue]
                                                               secondIndex:[_secondIndex[segmentIndex] integerValue]
                                                             selectedBlock:^(NSInteger index, NSInteger secondIndex, NSString *title) {
                                                                 [weakSelf changeIndex:index secondIndex:secondIndex andTitle:title withSegmentIndex:segmentIndex];
                                                             }];
        listView.tag = 9898;
...
- (void)changeIndex:(NSInteger)index secondIndex:(NSInteger)secondIndex andTitle:(NSString *)title withSegmentIndex:(NSInteger)segmentIndex
{
    if (index == -1) {
    ...
    [_secondIndex replaceObjectAtIndex:segmentIndex withObject:[NSNumber numberWithInteger:secondIndex]];
    
    // 是否想改变标题
    [_tabControl setTitle:title withIndex:segmentIndex];
