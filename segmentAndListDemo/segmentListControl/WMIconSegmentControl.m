...
- (void)setItemsWithIconArray:(NSArray *)iconArray selectedBlock:(WMIconSegmentControlBlock)selectedHandle
{
...
- (void)setItemsWithIconArray:(NSArray *)iconArray
{
    ...
    for (int i = 0; i < iconArray.count; i++) {
        ...
        if (iconPath.length > 0) {
            if (_imgDelegate && [_imgDelegate respondsToSelector:@selector(setWebImage:imgUrl:)]) {
...
{
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(WMIconSegmentControlDelegate)] && [self.delegate respondsToSelector:@selector(segmentControl:selectedIndex:)]) {
